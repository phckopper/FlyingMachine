...
#define ADDRESS_FRONT   ADDRESS_DEFAULT
#define ADDRESS_HEIGHT  0x10
...
uint8_t SELECTED_ADDRESS = ADDRESS_DEFAULT;
// Constructors ////////////////////////////////////////////////////////////////

// Public Methods //////////////////////////////////////////////////////////////
uint16_t read_front(void) {
	//return 0;
	SELECTED_ADDRESS = ADDRESS_FRONT;
	return readRangeContinuousMillimeters();
}

uint16_t read_height(void) {
	//return 0;
	SELECTED_ADDRESS = ADDRESS_HEIGHT;
	return readRangeContinuousMillimeters();
}
...
void distance_sensors_init(void) {
	...
	uint32_t s = HAL_I2C_IsDeviceReady(&hi2c2, ADDRESS_HEIGHT, 1, 0xff);
	if(s != HAL_OK) {
		vl53l0x_init(true);
		setAddress(ADDRESS_HEIGHT);
	}
...
void setAddress(uint8_t new_addr) {
	writeReg(I2C_SLAVE_DEVICE_ADDRESS, (new_addr & 0x7F) >> 1);
}
...
void writeReg16Bit(uint8_t reg, uint16_t value) {
	uint8_t w2_data[2];
	//w2_data[0] = reg;
	w2_data[0] = (value >> 8) & 0xFF;
	w2_data[1] = value & 0xFF;
...
void writeReg32Bit(uint8_t reg, uint32_t value) {
	uint8_t w2_data[4];
	//w2_data[0] = reg;
	w2_data[0] = (value >> 24) & 0xFF;
	w2_data[1] = (value >> 16) & 0xFF;
	w2_data[2] = (value >> 8) & 0xFF;
	w2_data[3] = value & 0xFF;
	//nrf_drv_twi_tx(&m_twi, SELECTED_ADDRESS, w2_data, sizeof(w2_data), false);
	HAL_I2C_Mem_Write(&hi2c2, SELECTED_ADDRESS, reg, I2C_MEMADD_SIZE_8BIT, w2_data,
					sizeof(w2_data), 0xff);
}
...
uint8_t readReg(uint8_t reg) {
	...
	//transfer_succeeded  = nrf_drv_twi_tx(&m_twi, SELECTED_ADDRESS, &reg, 1, true);
	//transfer_succeeded &= nrf_drv_twi_rx(&m_twi, SELECTED_ADDRESS, &value, 1);
...
uint16_t readReg16Bit(uint8_t reg) {
	...
	//bool transfer_succeeded;
	//transfer_succeeded  = nrf_drv_twi_tx(&m_twi, SELECTED_ADDRESS, &reg, 1, true);
	//transfer_succeeded &= nrf_drv_twi_rx(&m_twi, SELECTED_ADDRESS, buf, sizeof(buf));
	HAL_I2C_Mem_Read(&hi2c2, SELECTED_ADDRESS, reg, I2C_MEMADD_SIZE_8BIT, buf,
			sizeof(buf), 0xff);
...
void writeMulti(uint8_t reg, uint8_t const * src, uint8_t count) {
	//nrf_drv_twi_tx(&m_twi, SELECTED_ADDRESS, &reg, 1, true);
...
__attribute__ ((unused)) uint16_t decodeTimeout(uint16_t reg_val) {
...
__attribute__ ((unused)) uint16_t encodeTimeout(uint16_t timeout_mclks) {
	// format: "(LSByte * 2^MSByte) + 1"
...
__attribute__ ((unused)) uint32_t timeoutMclksToMicroseconds(
...
__attribute__ ((unused)) uint32_t timeoutMicrosecondsToMclks(
